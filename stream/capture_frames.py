#!/usr/bin/env python3
"""
stream/capture_frames.py - Captura frames do stream de câmera e os salva em dataset/raw/.
...
def parse_args():
    ...
    p.add_argument("--source", default="0", help="Índice da câmera (0, 1) ou 'mjpeg' para stream HTTP")
    p.add_argument("--url", default="http://localhost:5001/stream", help="URL do stream MJPEG (usado quando --source mjpeg)")
    p.add_argument("--total", type=int, default=200, help="Total de frames a capturar")
    p.add_argument("--interval", type=float, default=1.5, help="Intervalo entre capturas em segundos")
    p.add_argument("--width", type=int, default=640)
    p.add_argument("--height", type=int, default=480)
    p.add_argument("--manual", action="store_true", help="Captura manual: pressione ENTER a cada frame, em vez de intervalo automático")
    p.add_argument("--snapshot-url", default="http://localhost:5001/snapshot", help="URL do endpoint /snapshot (usado apenas com --source mjpeg --manual)")
    return p.parse_args()
...
def main():
    ...
    try:
        while saved < args.total:
            ...
            print(f" [{saved:>3}/{args.total}] Salvo: {path.name} (descartados: {skipped})", end=end_char)
